...
public class DysonSphereModuleTypeSelectionPanel extends GUIInputDialogPanel {
    ...
    public DysonSphereModuleTypeSelectionPanel(InputState state, GUICallback guiCallback, SuperStructureData structureData) {
        super(state, "DysonSphereModuleTypeSelectionPanel", "Construct new module", "Select a module type to construct.", 750, 400, guiCallback);
    ...
    public void onInit() {
        ...
        GUITilePane<GUIOverlay> modulePane = new GUITilePane<>(getState(), background, 132, 165);
        modulePane.onInit();
    ...
    private void createModuleTiles(GUITilePane<GUIOverlay> tilePane) {
        ...
        powerOverlay.getPos().y += 50;

        GUITile resourceTile = tilePane.addButtonTile("RESOURCES", "Generates resources by siphoning them from the star.", GUIHorizontalArea.HButtonColor.BLUE, new GUICallback() {
            @Override
            public void callback(GUIElement guiElement, MouseEvent mouseEvent) {
                if(mouseEvent.pressedLeftMouse()) {
                    //Todo: Queue module construction
